...
function vectorMulti(mp::JuMP.Model, C, v)

    n = length(v)
    multiExpression = [JuMP.@expression(mp, sum(C[i, j] * v[j] for j in 1:n)) for i in 1:n]
...
function inverseMat(originMat, connection)
    ...
    if LA.norm(originMat) > 0
        try
            invOri = LA.inv(originMat)
...
function obtainA_inverter_global(mpData, opfSol, rN, omega0, busList, invList, invLine, invConnected, inverters, invBusDict)
    ...
    for i in inverters
        ...
        iq0 = zeros(3)
        for j in 1:3
            id0[j], iq0[j] = LA.inv([vd0[j] vq0[j]; vq0[j] -vd0[j]]) * [P0 / 3; Q0 / 3]
        end
        dvd_delta = -vq0
        dvq_delta = vd0
        dvd_v = cos.(opfSol["solution"]["bus"][i]["va"])
        dvq_v = sin.(opfSol["solution"]["bus"][i]["va"])
        dP_delta = LA.dot(dvd_delta, id0) + LA.dot(dvq_delta, iq0)
        dQ_delta = -LA.dot(dvd_delta, iq0) + LA.dot(dvq_delta, id0)
        dP_v = LA.dot(dvd_v, id0) + LA.dot(dvq_v, iq0)
        dQ_v = -LA.dot(dvd_v, iq0) + LA.dot(dvq_v, id0)
        dP_id = vd0
        dP_iq = vq0
        dQ_id = vq0
        dQ_iq = -vd0
        ...
        A[i, i][4:6, 1] = LA.inv(L) * dvd_delta
        A[i, i][4:6, 3] = LA.inv(L) * dvd_v
        A[i, i][4:6, 4:6] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
        A[i, i][4:6, 7:9] = LA.inv(L) * link["br_x"]
        A[i, i][7:9, 1] = LA.inv(L) * dvq_delta
        A[i, i][7:9, 3] = LA.inv(L) * dvq_v
        A[i, i][7:9, 4:6] = -LA.inv(L) * link["br_x"]
        A[i, i][7:9, 7:9] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
    end
        ...
        if length(invConnected[iBus]) > 1
            for i in 1:length(invConnected[iBus])
                for j in (i+1):length(invConnected[iBus])
                    ...
                    link = mpData["branch"][invLine[i]]
                    L = link["br_x"] ./ omega0
                    A[i, j][4:6, 4:6] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
                    A[i, j][7:9, 7:9] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)

                    link = mpData["branch"][invLine[j]]
                    L = link["br_x"] ./ omega0
                    A[j, i] = zeros(9, 9)
                    A[j, i][4:6, 4:6] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
                    A[j, i][7:9, 7:9] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
                end
            end
        end
    end
...
function obtainA_inverter_global_var(mpData, pm, rN, omega0, busList, invList, invLine, invConnected, inverters, invBusDict)
    ...
    for iBus in inverters
        ...
        vd0 = [JuMP.@expression(pm.model, vmList.data[j] * cos(vaList.data[j])) for j in 1:length(vmList.data)]
        vq0 = [JuMP.@expression(pm.model, vmList.data[j] * sin(vaList.data[j])) for j in 1:length(vmList.data)]
        P0 = JuMP.@expression(pm.model, 0)
        Q0 = JuMP.@expression(pm.model, 0)
        ...
        for j in 1:3
            id0[j] = JuMP.@expression(pm.model, (vd0[j] * P0 / 3 + vq0[j] * Q0 / 3) / (vmList.data[j]^2))
            iq0[j] = JuMP.@expression(pm.model, (vq0[j] * P0 / 3 - vd0[j] * Q0 / 3) / (vmList.data[j]^2))
        end

        dvd_delta = [JuMP.@expression(pm.model, -vq0[j]) for j in 1:length(vq0)]
        dvq_delta = [JuMP.@expression(pm.model, vd0[j]) for j in 1:length(vd0)]
        dvd_v = [JuMP.@expression(pm.model, cos(vaList.data[j])) for j in 1:length(vaList.data)]
        dvq_v = [JuMP.@expression(pm.model, sin(vaList.data[j])) for j in 1:length(vaList.data)]
        dP_delta = JuMP.@expression(pm.model, sum(dvd_delta[j] * id0[j] for j in 1:length(dvd_delta)) + sum(dvq_delta[j] * iq0[j] for j in 1:length(dvq_delta)))
        dQ_delta = JuMP.@expression(pm.model, -sum(dvd_delta[j] * iq0[j] for j in 1:length(dvd_delta)) + sum(dvq_delta[j] * id0[j] for j in 1:length(dvq_delta)))
        dP_v = JuMP.@expression(pm.model, sum(dvd_v[j] * id0[j] for j in 1:length(dvd_v)) + sum(dvq_v[j] * iq0[j] for j in 1:length(dvq_v)))
        dQ_v = JuMP.@expression(pm.model, -sum(dvd_v[j] * iq0[j] for j in 1:length(dvd_v)) + sum(dvq_v[j] * id0[j] for j in 1:length(dvq_v)))
        dP_id = vd0
        dP_iq = vq0
        dQ_id = vq0
        dQ_iq = [JuMP.@expression(pm.model, -vd0[i]) for i in 1:length(vd0)]

        A[iBus, iBus][1, 2] = 1
        A[iBus, iBus][2, :] = [JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_delta)
            -1 / mpData["bus"][iBus]["tau"]
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_v)
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_id[1])
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_id[2])
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_id[3])
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_iq[1])
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_iq[2])
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mp"] * dP_iq[3])
        ]
        A[iBus, iBus][3, :] = [JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * mpData["bus"][iBus]["mq"] * dQ_delta)
            0
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (1 + mpData["bus"][iBus]["mq"] * dQ_v))
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (mpData["bus"][iBus]["mq"] * dQ_id[1]))
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (mpData["bus"][iBus]["mq"] * dQ_id[2]))
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (mpData["bus"][iBus]["mp"] * dQ_id[3]))
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (mpData["bus"][iBus]["mq"] * dQ_iq[1]))
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (mpData["bus"][iBus]["mq"] * dQ_iq[2]))
            JuMP.@expression(pm.model, -1 / mpData["bus"][iBus]["tau"] * (mpData["bus"][iBus]["mp"] * dQ_iq[3]))
        ]
        A[iBus, iBus][4:6, 1] = PMS.vectorMulti(pm.model, LA.inv(L), dvd_delta)
        A[iBus, iBus][4:6, 3] = PMS.vectorMulti(pm.model, LA.inv(L), dvd_v)
        A[iBus, iBus][4:6, 4:6] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
        A[iBus, iBus][4:6, 7:9] = LA.inv(L) * link["br_x"]
        A[iBus, iBus][7:9, 1] = PMS.vectorMulti(pm.model, LA.inv(L), dvq_delta)
        A[iBus, iBus][7:9, 3] = PMS.vectorMulti(pm.model, LA.inv(L), dvq_v)
        A[iBus, iBus][7:9, 4:6] = -LA.inv(L) * link["br_x"]
        A[iBus, iBus][7:9, 7:9] = -LA.inv(L) * link["br_r"] - rN * LA.inv(L)
    end
...
function get_global_stability_matrix(mpData, opfSol, rN, omega0)

    if isapprox(omega0, 0, atol=1E-6)
        Memento.error(_LOGGER, "Inverter frequency set-point value cannot be zero")
    end
